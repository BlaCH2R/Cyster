// Identify exactly which storyboard sprite paints the playfield red.
// Reproduces the exact flow that showed red (refreshAll + setTime + render),
// then dumps every visible sprite and samples canvas pixels.
const { app, BrowserWindow } = require('electron');
const fs = require('fs');
const path = require('path');
const os = require('os');
app.setPath('userData', fs.mkdtempSync(path.join(os.tmpdir(), 'cytoid_sb_red_ud_')));
...
app.whenReady().then(async () => {
  ...
  const out = await win.webContents.executeJavaScript(`(async () => {
    ...
    // Reproduce the flow that rendered the red playfield.
    ...
    for (const r of (ev.sprites || [])) {
      ...
      const img = f.path ? pv.imageCache[f.path] : null;
      all.push({
        id: r.obj.id,
        path: f.path,
        t: f.time,
    ...
    return {
      W, H, samples,
      visible: visible.sort((a, b) => (a.matrix[5] || 0) - (b.matrix[5] || 0)),
      bgImageLoaded: !!(pv.backgroundImage && pv.backgroundImage.complete && pv.backgroundImage.naturalWidth > 0)
    };
  })()`);
  console.log('RED-PROBE:', JSON.stringify(out));
  app.exit(0);
});
