// Text-rendering comparison harness:
// A) self-built preview frame at t=0 (official fonts loaded)
// B) real Unity engine frame at t=0 (seek + pause)
...
const PIPE = 'cytoid_sb_text_' + Date.now().toString(36);
const SELF_PNG = 'V:\\cytoid storyboarder\\tools\\text_self.png';
const ENGINE_PNG = 'V:\\cytoid storyboarder\\tools\\text_engine.png';
...
app.setPath('userData', fs.mkdtempSync(path.join(os.tmpdir(), 'cytoid_sb_text_')));
...
app.whenReady().then(async () => {
  ...
  // A) Self-built preview
  const info = buildInfo(LEVEL);
  await win.webContents.executeJavaScript(`window.__sb.loadLevelInfo(${JSON.stringify(info)})`);
  await new Promise((r) => setTimeout(r, 4000)); // allow fonts to load
  const fonts = await win.webContents.executeJavaScript(`document.fonts.status`);
  ...
  console.log('self frame saved, fonts status =', fonts);
  ...
  // Find the engine window and capture it.
  const pid = engine.pid;
  const wins = execFileSync('powershell.exe', [
    '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
    path.join(__dirname, 'check_win_size.ps1'), '-TargetPid', String(pid)
  ], { encoding: 'utf8' });
  const match = /win\d+=(\d+)x(\d+)/.exec(wins);
  if (!match) { console.log('FAIL: no engine window:', wins); engine.kill(); app.exit(1); }
  // The check script prints rects of top-level windows; capture using GetWindowRect
  // coordinates instead — use a dedicated capture that finds the window itself.
  const rectOut = execFileSync('powershell.exe', [
    '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
    path.join(__dirname, 'capture_engine.ps1'),
    '-TargetPid', String(pid), '-Out', ENGINE_PNG
  ], { encoding: 'utf8' });
  console.log('engine frame:', rectOut.trim());

  engine.kill();
  console.log('TEXT_COMPARE_DONE');
  app.exit(0);
